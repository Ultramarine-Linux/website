 ...
 * </Trans>
 * ```
 * With `"test": "Hello <1>{{user}}</1>, welcome to <2>my site</2>."`, the above element will become:
 * ```tsx
 * <b>Hello John</b>, welcome to <a href="https://example.com">my site</a>.
 * ```
 * 
 ...
 * @returns ReactNode
 */
const Trans: React.FunctionComponent<Props> = ({
  children,
  t,
}: Props) => {
  ...
  const realchildren: ReactElement<PropsWithChildren>[] = Children.toArray(
    children,
  ...
  for (;;) {
    ...
    if (t.substring(ch_idx, ch_idx + 2) == "</") {
      let j = 0;
      while (t[++j + ch_idx] != ">");
      const tagNumber = Number.parseInt(
        t.substring(ch_idx + 1, ch_idx + j),
      );
      if (Number.isNaN(tagNumber)) {
        contents.push(t.substring(ch_idx, ch_idx + j + 1));
        ch_idx += j + 1;
        continue;
      }
      ...
      realchildren[p].props.children = (
        <>{contents.splice(l, contents.length - l)}</>
      );
      contents.push(realchildren[p]);
      continue;
    }
    if (t[ch_idx] == "<") {
      let j = 0;
